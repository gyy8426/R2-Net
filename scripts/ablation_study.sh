...
if [ $1 == "0" ]; then
    ...
    python3 -u models/train_rels.py -m sgcls -model motifnet -order leftright -nl_obj 2 -nl_edge 4 -nl_adj 0 -b 24 -clip 5\
        ...
        -save_dir  ${SAVE_MODEL_PATH}/ablation_study/rel_model_edgelstm_regfeat_objedgepos/ \
    ...
    python3 -u models/train_rels.py -m sgcls -model motifnet -order leftright -nl_obj 2 -nl_edge 4 -nl_adj 0 -b 24 -clip 5\
        -pooling_dim 2048 -hidden_dim 512 -adj_embed_dim 512 \
        -lr 2e-2 -ngpu 1 -ckpt ${SAVE_MODEL_PATH}/ablation_study/refine_rel_model_edgelstm_regfeat/vgrel-16.tar \
        -save_dir  ${SAVE_MODEL_PATH}/ablation_study/refine_rel_model_edgelstm_regfeat/ \
        -nepoch 50 -resnet -p 100 -use_bias \
        -with_adj_mat -bg_num_graph 2 -bg_num_rel 1 -with_gcn -with_biliner_score -gcn_adj_type soft -where_gcn stack
elif [ $1 == "2" ]; then
    echo "rel_model_nolstm2"
    export CUDA_VISIBLE_DEVICES=1
    python3 -u models/train_rels.py -m sgcls -model motifnet -order leftright -nl_obj 2 -nl_edge 0 -nl_adj 0 -b 24 -clip 5\
        -pooling_dim 2048 -hidden_dim 512 -adj_embed_dim 512 \
        ...
        -save_dir  ${SAVE_MODEL_PATH}/ablation_study/rel_model_nolstm2/ \
        -nepoch 50 -resnet -p 100 -use_bias \
        -with_adj_mat -bg_num_graph 2 -bg_num_rel 1 -with_gcn -with_biliner_score -gcn_adj_type soft -where_gcn stack
elif [ $1 == "3" ]; then
    ...
    echo "rel_model_gcnfront"
    export CUDA_VISIBLE_DEVICES=1
    python3 -u models/train_rels.py -m sgcls -model motifnet -order leftright -nl_obj 2 -nl_edge 4 -nl_adj 0 -b 24 -clip 5\
        -pooling_dim 2048 -hidden_dim 512 -adj_embed_dim 512 \
        -lr 2e-2 -ngpu 1 -ckpt /mnt/data1/guoyuyu/datasets/visual_genome/model/neural-motifs_nongt/checkpoints/vgdet_res101_layer4_finished/vg-24.tar \
        -save_dir  ${SAVE_MODEL_PATH}/ablation_study/rel_model_gcn_front/ \
        -nepoch 50 -resnet -p 100 -use_bias \
        -with_adj_mat -bg_num_graph 2 -bg_num_rel 1 \
        -with_gcn -with_biliner_score -gcn_adj_type soft -where_gcn front
fi
