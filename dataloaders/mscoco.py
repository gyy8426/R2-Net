...
class CocoDetection(Dataset):
    ...
    def __init__(self, mode):
        ...
        self.ann_file = os.path.join(COCO_PATH, 'annotations', 'annot_tra_val', 'instances_{}.json'.format(mode))
...
def coco_collate(data, num_gpus=3, is_train=False, mode='det'):
...
class CocoDataLoader(torch.utils.data.DataLoader):
    ...
    @classmethod
    def splits(cls, train_data, val_data, batch_size=3, num_workers=1, num_gpus=3, mode='det', shuffle=True, **kwargs):
        assert mode in ('det', 'rel')
        train_load = cls(
            dataset=train_data,
            ...
            collate_fn=lambda x: coco_collate(x, mode=mode, num_gpus=num_gpus, is_train=True),
            drop_last=True,
            # pin_memory=True,
            **kwargs,
        )
        val_load = cls(
            ...
            shuffle=False,
            num_workers=num_workers,
            collate_fn=lambda x: coco_collate(x, mode=mode, num_gpus=num_gpus, is_train=False),
            drop_last=True,
            # pin_memory=True,
            **kwargs,
        )
