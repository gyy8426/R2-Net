...
class Blob(object):
    def __init__(self, mode='det', is_train=False, num_gpus=1, primary_gpu=0, batch_size_per_gpu=3):
        ...
        self.gt_adj_mat = []  # [num_gt, 65]. Each row is (gtbox0, gtbox1, rel).
        self.ae_adj_mat_rel = []  # [num_gt, 65]. Each row is (gtbox0, gtbox1, rel).
        self.ae_adj_mat_obj = []  # [num_gt, 65]. Each row is (gtbox0, gtbox1, rel).
        self.ae_pred_rel_inds = []  # [num_gt, 65]. Each row is (gtbox0, gtbox1, rel).
        self.gt_sents = []
        self.gt_nodes = []
        self.sent_lengths = []
    ...
    def append(self, d):
        ...
        self.img_inds.append(d['image_id'])
        ...
        self.num_box.append(d['num_box'])
        self.gt_mul_label.append(d['gt_mul_label'])
        self.gt_mul_rel.append(d['gt_mul_rel'])
        self.gt_mul_label_num.append(d['gt_mul_label_num'])


        self.ae_adj_mat_rel.append(d['ae_adj_mat_rel'])
        self.ae_adj_mat_obj.append(d['ae_adj_mat_obj'])
        self.ae_pred_rel_inds.append(d['ae_pred_rel_inds'])
    ...
    def reduce(self):
        ...
        self.img_inds = np.stack(self.img_inds).reshape(
            (self.num_gpus, self.batch_size_per_gpu))
        if self.is_rel:
            self.gt_rels, self.gt_rel_chunks = self._chunkize(self.gt_rels)
            self.gt_adj_mat, self.gt_adj_mat_chunks = self._chunkize(self.gt_adj_mat)
        ...
        self.gt_mul_label, _ = self._chunkize(self.gt_mul_label,tensor=torch.FloatTensor)
        self.num_box, _ = self._chunkize(self.num_box)
        self.gt_mul_rel, _ = self._chunkize(self.gt_mul_rel, tensor=torch.FloatTensor)
        self.gt_mul_label_num, _ = self._chunkize(self.gt_mul_label_num)

        self.ae_adj_mat_rel, _ = self._chunkize(self.ae_adj_mat_rel, tensor=torch.FloatTensor)
        self.ae_adj_mat_obj, _ = self._chunkize(self.ae_adj_mat_obj, tensor=torch.FloatTensor)
        self.ae_pred_rel_inds, _ = self._chunkize(self.ae_pred_rel_inds)
    ...
    def scatter(self):
        ...
        self.gt_mul_rel = self._scatter(self.gt_mul_rel, self.gt_box_chunks)
        self.gt_mul_label_num = self._scatter(self.gt_mul_label_num, self.gt_box_chunks)
        self.gt_mul_label = self._scatter(self.gt_mul_label, self.gt_box_chunks)
        self.num_box = self._scatter(self.num_box, self.gt_box_chunks)

        self.ae_adj_mat_rel = self._scatter(self.ae_adj_mat_rel, self.gt_box_chunks)
        self.ae_adj_mat_obj = self._scatter(self.ae_adj_mat_obj, self.gt_box_chunks)
        self.ae_pred_rel_inds = self._scatter(self.ae_pred_rel_inds, self.gt_box_chunks)
        ...
        if self.is_train:
            ...
            if self.is_rel:
                ...
                self.gt_adj_mat = self._scatter(self.gt_adj_mat,
                                                self.gt_adj_mat_chunks)

        else:
            if self.is_rel:
                self.gt_rels = self.gt_rels.cuda(self.primary_gpu, async=True)
                self.gt_adj_mat = self.gt_adj_mat.cuda(self.primary_gpu, async=True)
    ...
    def __getitem__(self, index):
        ...
        if self.is_rel:
            rels = self.gt_rels
            adj_mats = self.gt_adj_mat
            if index > 0 or self.num_gpus != 1:
                rels_i = rels[index] if self.is_rel else None
                adj_mats_i = adj_mats[index] if self.is_rel else None
        elif self.is_flickr:
        ...
        if index == 0 and self.num_gpus == 1:
            image_offset = 0
            if self.is_train:
                return (self.img_inds, self.imgs, self.im_sizes[0], image_offset,
                        self.gt_boxes, self.gt_classes, adj_mats, rels,
                        self.gt_mul_label, self.gt_mul_rel, self.gt_mul_label_num,
                        self.num_box,
                        proposals,
                        self.ae_adj_mat_rel,self.ae_adj_mat_obj,self.ae_pred_rel_inds,
                        self.train_anchor_inds)
            return self.img_inds, self.imgs, self.im_sizes[0], image_offset, self.gt_boxes, self.gt_classes, \
                   adj_mats, rels,\
                   self.gt_mul_label, self.gt_mul_rel, self.gt_mul_label_num, self.num_box,\
                   proposals,\
                   self.ae_adj_mat_rel, self.ae_adj_mat_obj, self.ae_pred_rel_inds,
        ...
        if self.is_train:
            return (
                self.img_inds, self.imgs[index], self.im_sizes[index], image_offset,
            self.gt_boxes[index], self.gt_classes[index], adj_mats_i, rels_i,
            self.gt_mul_label[index], self.gt_mul_rel[index], self.gt_mul_label_num[index],
            self.num_box[index],
                None, self.ae_adj_mat_rel[index], self.ae_adj_mat_obj[index], self.ae_pred_rel_inds[index], self.train_anchor_inds[index])
        return (self.img_inds, self.imgs[index], self.im_sizes[index], image_offset,
                self.gt_boxes[index], self.gt_classes[index], adj_mats_i, rels_i,
                self.gt_mul_label[index], self.gt_mul_rel[index], self.gt_mul_label_num[index],
                self.num_box[index],
                None, self.ae_adj_mat_rel[index], self.ae_adj_mat_obj[index], self.ae_pred_rel_inds[index])
