    ...
    if conf.mode == 'detclass':
        ...
        if conf.with_adj_mat and not conf.gt_adj_mat:
            ...
            loss_in_gt_bg_score = result.gt_adj_mat_graph[result.rel_labels_graph[:, 1], \
                                                          result.rel_labels_offset_graph[:, 2]].type(
                torch.cuda.LongTensor)

            losses['graph_loss'] = F.cross_entropy(loss_in_pre_bg_score, loss_in_gt_bg_score)
        loss = sum(losses.values())
    else:
        if conf.att_alpha_c > 0. and result.att_alpha != None:
            losses['alpha_loss'] = conf.att_alpha_c * (((1. - result.att_alpha.sum(0)) ** 2).sum(-1) / conf.batch_size)
        if conf.with_adj_mat and not conf.gt_adj_mat:
            ...
            if result.pre_adj_mat_rel is not None:
                ...
                loss_in_gt_bg_score = result.gt_adj_mat_graph[result.rel_labels_graph[:, 1], \
                                                              result.rel_labels_offset_graph[:, 2]].type(
                    torch.cuda.LongTensor)
                loss_in_gt_bg_1 = ((loss_in_gt_bg_score.type_as(loss_in_pre_bg_score_rel)).sum()) / (
                            1.0 * loss_in_gt_bg_score.size(0))
...
for epoch in range(start_epoch + 1, start_epoch + 1 + conf.num_epochs):
    ...
    adj_obj_recall, adj_rel_recall = val_epoch()
