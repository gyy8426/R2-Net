    ...
    if conf.mode != 'detclass':
        for i, (boxes_i, objs_i, obj_scores_i, rels_i, pred_scores_i, \
            ...
            if conf.mode != 'sgdet':
                ...
                num_correct_adj_mat_rel = num_correct_adj_mat_rel + np.sum(
                    (gt_adj_mat_i - pred_adj_mat_bool_rel_i) == 0)
                num_correct_adj_mat_obj = num_correct_adj_mat_obj + np.sum(
                    (gt_adj_mat_i - pred_adj_mat_bool_obj_i) == 0)
                ...
                if pred_adj_mat_bool_rel_i != 0  and pred_adj_mat_bool_obj_i != 0:
                    for gt_rels_i in gt_rel_k:
                        all_rel_num[gt_rels_i[2]] = all_rel_num[gt_rels_i[2]] + 1
                        for id_pred_rel, pred_rel_i in enumerate(rels_i):
                            if pred_rel_i[0] == gt_rels_i[0] and pred_rel_i[1] == gt_rels_i[1] \
                            and pred_adj_mat_bool_rel_i[id_pred_rel] == 1:
                                all_TP_rel_rel_num[gt_rels_i[2]] = all_TP_rel_rel_num[gt_rels_i[2]] + 1
                                break
                        for id_pred_rel, pred_rel_i in enumerate(rels_i):
                            if pred_rel_i[0] == gt_rels_i[0] and pred_rel_i[1] == gt_rels_i[1] \
                            and pred_adj_mat_bool_obj_i[id_pred_rel] == 1:
                                all_TP_rel_obj_num[gt_rels_i[2]] = all_TP_rel_obj_num[gt_rels_i[2]] + 1
                                break
            else:
                num_correct_adj_mat_obj = 0.0
                num_correct_adj_mat_rel = 0.0
...
if conf.cache is not None and os.path.exists(conf.cache):
    ...
    for val_b, batch in enumerate(tqdm(val_loader)):
        ...
        True_sample_i, TP_sample_obj_i, TP_sample_rel_i  = val_batch(conf.num_gpus*val_b, batch, evaluator)
