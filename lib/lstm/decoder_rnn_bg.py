...
class DecoderRNN(torch.nn.Module):
    def __init__(self, classes, embed_dim, inputs_dim, hidden_dim, recurrent_dropout_probability=0.2,
                 use_highway=True, use_input_projection_bias=True, type='obj', mode='sgcls'):
        """
        Initializes the RNN
    ...
    def forward(self,  # pylint: disable=arguments-differ
                ...
                labels=None, boxes_for_nms=None, type='obj', rel_inds=None, obj_dists=None, obj_preds=None):
        """
        Parameters
        ----------
        ...
        previous_embed = F.dropout(previous_embed, self.recurrent_dropout_probability, training=self.training)
        ...
        out_dists = []
        out_commitments = []
        out_commitments_nozeros = []
        out_commitments_zeros = []
        out_state = []
        end_ind = 0
        for i, l_batch in enumerate(batch_lengths):
            ...
            pred_dist = self.out(previous_state) + obj_dists[start_ind:end_ind]
            ...
            if self.training:
                labels_to_embed_zeros = labels[start_ind:end_ind].clone()
                # Whenever labels are 0 set input to be our max prediction
                out_commitments_zeros.append(labels_to_embed_zeros.clone())

                labels_to_embed_nozeros = labels[start_ind:end_ind].clone()
                nonzero_pred = pred_dist[:, 1:].max(1)[1] + 1
                is_bg = (labels_to_embed_nozeros.data == 0).nonzero()
                if is_bg.dim() > 0:
                    labels_to_embed_nozeros[is_bg.squeeze(1)] = nonzero_pred[is_bg.squeeze(1)]
                out_commitments_nozeros.append(labels_to_embed_nozeros)


                if self.mode == 'sgdet':
                    previous_embed = self.obj_embed(labels_to_embed_zeros + 1)
                    #previous_embed = self.obj_embed(labels_to_embed_nozeros + 1)
                else:
                    previous_embed = self.obj_embed(labels_to_embed_nozeros + 1)
                previous_embed = F.dropout(previous_embed, self.recurrent_dropout_probability, training=self.training)
            else:
                ...
                best_ind_nozero = out_dist_sample[:, 1:].max(1)[1] + 1
                best_ind_zero = out_dist_sample[:, :].max(1)[1]
                ...
                out_commitments_nozeros.append(best_ind_nozero)
                out_commitments_zeros.append(best_ind_zero)
                if self.mode == 'sgdet':
                    #previous_embed = self.obj_embed(best_ind_nozero + 1)
                    previous_embed = self.obj_embed(best_ind_zero + 1)
                else:
                    previous_embed = self.obj_embed(best_ind_nozero + 1)


        # Do NMS here as a post-processing step
        if (boxes_for_nms is not None and not self.training) and type == 'obj' :
            ...
            out_dists_sampled_zero = out_dists_sampled.copy()
            out_dists_sampled_nozero = out_dists_sampled.copy()
            out_dists_sampled_nozero[:,0] = 0

            out_commitments_nozeros = out_commitments_nozeros[0].data.new(len(out_commitments_nozeros)).fill_(0)

            for i in range(out_commitments_nozeros.size(0)):
                box_ind, cls_ind = np.unravel_index(out_dists_sampled_nozero.argmax(), out_dists_sampled_nozero.shape)
                out_commitments_nozeros[int(box_ind)] = int(cls_ind)
                out_dists_sampled_nozero[is_overlap[box_ind, :, cls_ind], cls_ind] = 0.0
                out_dists_sampled_nozero[box_ind] = -1.0  # This way we won't re-sample

            out_commitments_nozeros = Variable(out_commitments_nozeros)

            out_commitments_zeros = out_commitments_zeros[0].data.new(len(out_commitments_zeros)).fill_(0)

            for i in range(out_commitments_zeros.size(0)):
                box_ind, cls_ind = np.unravel_index(out_dists_sampled_zero.argmax(), out_dists_sampled_zero.shape)
                out_commitments_zeros[int(box_ind)] = int(cls_ind)
                out_dists_sampled_zero[is_overlap[box_ind, :, cls_ind], cls_ind] = 0.0
                out_dists_sampled_zero[box_ind] = -1.0  # This way we won't re-sample

            out_commitments_zeros = Variable(out_commitments_zeros)


        else:
            #out_commitments = torch.cat(out_commitments, 0)
            out_commitments_nozeros = torch.cat(out_commitments_nozeros, 0)
            out_commitments_zeros = torch.cat(out_commitments_zeros, 0)

        return torch.cat(out_dists, 0), out_commitments_nozeros, out_commitments_zeros, torch.cat(out_state,0)
