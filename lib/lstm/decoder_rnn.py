...
class DecoderRNN(torch.nn.Module):
    ...
    def forward(self,  # pylint: disable=arguments-differ
        ...
        for i, l_batch in enumerate(batch_lengths):
            ...
            if obj_dists is not None:
                pred_dist = self.out(previous_state) + obj_dists[start_ind:end_ind]
            else:
                pred_dist = self.out(previous_state)
            if type == 'union':
                pred_dist = pred_dist + self.freq_bias.index_with_labels(torch.stack((
                    obj_preds[rel_inds[start_ind:end_ind, 1]],
                    obj_preds[rel_inds[start_ind:end_ind, 2]],
                ), 1))
            ...
            if self.training:
                ...
                if type != 'union':
                    ...
                    nonzero_pred = pred_dist.max(1)[1]
        ...
        return torch.cat(out_dists, 0), out_commitments, out_commitments, torch.cat(out_state,0)
