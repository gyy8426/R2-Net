...
def anchor_target_layer(gt_boxes, im_size, 
    ...
    anchor_to_gtbox = overlaps.argmax(axis=1) # #num_anchors
    max_overlaps = overlaps[np.arange(anchor_to_gtbox.shape[0]), anchor_to_gtbox]
    gtbox_to_anchor = overlaps.argmax(axis=0) # #num_gtboxes
    gt_max_overlaps = overlaps[gtbox_to_anchor, np.arange(overlaps.shape[1])] # #num_gtboxes
