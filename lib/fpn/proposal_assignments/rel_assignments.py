...
def rel_assignments(im_inds, rpn_rois, roi_gtlabels, gt_boxes, gt_classes, gt_rels, image_offset,
                    fg_thresh=0.5, num_sample_per_gt=4, filter_non_overlap=True, max_obj_num=65,
                    time_bg=-1, time_fg=-1):
    """
    ...
    num_box_seen = 0
    offset_array = []
    gt_adj_mat = np.array([]).reshape(0,max_obj_num)
    for im_ind in range(num_im):
        offset_array.append(num_box_seen)
        pred_ind = np.where(pred_inds_np == im_ind)[0]
        #pred_ind  the box_ids in im_ind
        ...
        gt_adj_mat_i = np.zeros(shape=[pred_ind.shape[0], max_obj_num])
        # [num_pred, num_gt]
        ...
        gt_adj_mat_i[fg_rels[:,0],fg_rels[:,1]] = 1.0
        gt_adj_mat = np.concatenate([gt_adj_mat, gt_adj_mat_i],0)
        ...
        if time_bg == -2:
            num_bg_rel = num_bg_rel/2
        elif time_bg >= 0:
            num_bg_rel = time_bg * fg_rels.shape[0]
        ...
        if bg_rels.size > 0:
            ...
            # p /= p.sum()
            if num_bg_rel > bg_rels.shape[0]:
                num_bg_rel = bg_rels.shape[0]
            bg_rels = bg_rels[
        ...
        if fg_rels.size == 0 and bg_rels.size == 0:
            ...
            fg_rels = np.array([[0, 1, 31]], dtype=np.int64)  # 31 is on  29 is near
            bg_rels = np.array([[0, 0, 0]], dtype=np.int64)

        # print("GTR {} -> AR {} vs {}".format(gt_rels.shape, fg_rels.shape, bg_rels.shape))
        bg_num = bg_rels.size
        fg_num = fg_rels.size
        fg_freq = fg_num / (1.0 * fg_num + bg_num)
        if time_fg == 0:
            all_rels_i = bg_rels
        if time_bg == 0:
            all_rels_i = fg_rels
        if time_fg != 0 and time_bg != 0:
            all_rels_i = np.concatenate((fg_rels, bg_rels), 0)
        if all_rels_i.shape[0] == 0:
            all_rels_i = np.array([[0, 1, 31]], dtype=np.int64)  # 31 is on  # this case only for num_fg > 0
    ...
    offset_array = torch.LongTensor(offset_array).cuda(rpn_rois.get_device(), async=True)
    rel_labels = torch.LongTensor(np.concatenate(rel_labels, 0)).cuda(rpn_rois.get_device(),
                                                                          async=True)
    rel_labels_offset = rel_labels.clone()
    rel_labels_offset[:,1:3] = rel_labels_offset[:,1:3]-offset_array[rel_labels_offset[:,0]][:,None]
    gt_adj_mat = torch.FloatTensor(gt_adj_mat).cuda(rpn_rois.get_device(), async=True)

    return rel_labels, gt_adj_mat, rel_labels_offset
