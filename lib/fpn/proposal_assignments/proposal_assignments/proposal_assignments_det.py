...
def proposal_assignments_det(rpn_rois, gt_boxes, gt_classes, image_offset, fg_thresh=0.5):
    ...
    :param rpn_rois: [num_rpn_boxes, 5] array og [img_ind, x1, y1, x2, y2]
    :param gt_boxes:   [num_gt_boxes, 4] array of x0, y0, x1, y1
    :param gt_classes: [num_gt_boxes, 2] array of [img_ind, class]
    ...
    # all_boxes shape: num_rpn + num_gt_boxes, 4 (x1, y1, x2, y2)
    ims_per_box = torch.cat([rpn_rois[:, 0].long(), gt_img_inds], 0)
    # all_boxes shape: num_rpn + num_gt_boxes (img_ids)
    ...
    for im_ind in range(num_images):
        ...
        # box_inds whose image id is im_ind
        ...
        # ious num_all_boxes, num_gt_boxes
        max_overlaps, gt_assignment = ious.max(1)
        # max_overlaps: the size of max overlap shape: num_all_boxes. gt_assignment: the index of max overlap of gt class
        ...
        keep_inds_np, num_fg = _sel_inds(max_overlaps, fg_thresh, fg_rois_per_image,
                                         ROIS_PER_IMG)
        # keep_inds_np: shape: ROIS_PER_IMG  the boxes index of sample boxes
        # num_fg : the number of fore ground box
        if keep_inds_np.size == 0:
            continue

        keep_inds = torch.LongTensor(keep_inds_np).cuda(rpn_rois.get_device())

        labels_ = gt_classes[:, 1][gt_assignment[keep_inds]] # the class of the keep_inds boxes
        #equal labels_ = gt_classes[gt_assignment[keep_inds], 1]
