...
import torch
from lib.pytorch_misc import diagonal_inds, to_variable, generate_negative_samples
...
def proposal_assignments_gtbox(rois, gt_boxes, gt_classes, gt_rels, image_offset, fg_thresh=0.5,
                               time_bg=-1, time_fg=-1, time_neg=0):
    """
    ...
    offset_array = torch.LongTensor(offset_array).cuda(rois.get_device(),async=True)
    ...
    #negtive sample
    neg_rels = np.array([[0,0,0,0]])
    if time_neg > 0:
        for i, s, e in enumerate_by_image(fg_rels[:, 0]):
            neg_sample_i = generate_negative_samples(fg_rels[s:e], time_neg)
            if neg_sample_i.shape[0] > 0:
                neg_rels = np.concatenate([neg_rels, neg_sample_i],0)
    neg_rels = torch.LongTensor(neg_rels).contiguous().cuda(fg_rels.get_device(),async=True)
    ...
    rel_labels_offset = rel_labels_offset.contiguous()
    return rois, labels, rel_labels, rel_labels_offset
