...
class BasicSceneGraphEvaluator:
    ...
    def evaluate_scene_graph_entry(self, gt_entry, pred_scores, viz_dict=None, iou_thresh=0.5,
                                        with_beta=False, alpha=0.5, beta=0.5):
    ...
    def get_recall(self):
        kl = []
        for k, v in self.result_dict[self.mode + '_recall'].items():
            kl.append([k,np.mean(v)])
        return kl

def evaluate_from_dict(gt_entry, pred_entry, mode, result_dict, multiple_preds=False,
                       viz_dict=None, alpha=0.5, beta=0.2, with_beta=False, **kwargs):

    """
    Shortcut to doing evaluate_recall from dict
    when testing model, the batch_size always equal to one
    ...
    adj_mat_rel = pred_entry['pred_adj_mat_rel']
    adj_mat_obj = pred_entry['pred_adj_mat_obj']
    if mode == 'predcls' or mode == 'predcls_nongtbox':
        pred_boxes = gt_boxes
    ...
    if multiple_preds:
        ...
        if adj_mat_obj is not None and adj_mat_rel is not None:
            if adj_mat_obj is None and adj_mat_rel is not None:
                overall_scores = ((obj_scores_per_rel[:,None] * rel_scores[:,1:])** (1.0 - alpha))  * \
                                      ((adj_mat_rel) ** (alpha))[:, None]
            elif adj_mat_rel is None and adj_mat_obj is not None:
                overall_scores = ((obj_scores_per_rel[:,None] * rel_scores[:,1:])** (1.0 - alpha))  * \
                                      ((adj_mat_obj) ** (alpha))[:, None]
            else:
                if with_beta:
                    overall_scores = ((obj_scores_per_rel[:, None] * rel_scores[:, 1:]) ** (1.0 - alpha)) * \
                                     (((adj_mat_rel ** (beta)) * (adj_mat_obj ** ((1.0 - beta)))) ** (alpha))[:, None]
                else:
                    overall_scores = ((obj_scores_per_rel[:, None] * rel_scores[:, 1:]) ** (1.0 - alpha)) * \
                                     (((adj_mat_rel ** (1.0)) * (adj_mat_obj ** ((1.0)))) ** (alpha))[:, None]
