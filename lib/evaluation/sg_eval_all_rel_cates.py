...
class BasicSceneGraphEvaluator:
    ...
    def print_stats(self, epoch='final'):
        print('======================' + self.mode + '============================')
        rel_cat_recall = {}
        for k, v in self.result_dict[self.mode + '_recall'].items():
            ...
            for rel_cat_id, rel_cat_name in self.rel_cats.items():
                ...
                rel_cat_recall[k][rel_cat_name] = np.mean(v[rel_cat_name])
                sum_rel_call = sum_rel_call + rel_cat_recall[k][rel_cat_name]
            rel_cat_recall[k]['mean_rel_recall'] = sum_rel_call / (1.0 * len(self.rel_cats))
            print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        pkl.dump(rel_cat_recall, open('./output/rel_cat_recall_'+str(epoch)+'.npz','wb'))
