...
class LinearizedContext(nn.Module):
    ...
    def __init__(self, classes, rel_classes, mode='sgdet',
                 ...
                 edge_ctx_type='obj_ctx', nms_union=False):
        ...
        if self.nl_obj > 0 and self.mode != 'predcls_nongtbox':
            ...
            self.obj_ctx_rnn = AlternatingHighwayLSTM(
                input_size=obj_ctx_rnn_indim + 1024,
        ...
        if self.nl_obj == 0 or self.with_gcn:
            ...
            self.obj_gc1_re = GraphConvolution(obj_gcn_input_dim, self.hidden_dim)
            #self.obj_gc2 = GraphConvolution(self.hidden_dim, self.hidden_dim)
            #self.obj_gc2_re = GraphConvolution(self.hidden_dim, self.hidden_dim)
            #self.obj_gc1_fb = GraphConvolution(self.obj_dim + self.embed_dim + 128, self.hidden_dim)
        ...
        if self.with_gcn:
            ...
            for i in range(self.num_gcn_layer):
                ...
                self.gc_re_list.append(GraphConvolution(gcn_input_dim, self.hidden_dim))
                '''
                self.gc2_list.append(GraphConvolution(hidden_dim, self.hidden_dim))
                self.gc2_re_list.append(GraphConvolution(hidden_dim, self.hidden_dim))
                '''
    ...
    def edge_ctx(self, obj_feats, obj_dists, im_inds, obj_preds, box_priors=None, feat_im_inds=None,
                 union_feat=None, union_box=None, adj_dis_softmax=None, rel_inds=None, rel_label=None):
        """
        ...
        obj_embed2 = F.dropout(obj_embed2, self.dropout_rate, training=self.training)
        obj_att_w = None
        confidence = F.softmax(obj_dists, dim=1).data.view(-1)[
            obj_preds.data + arange(obj_preds.data) * self.num_classes]
        s_obj_feats = obj_feats
        # obj_embed3 = F.softmax(obj_dists, dim=1) @ self.obj_embed3.weight
        if self.mode == 'predcls_nongtbox':
            obj_feat_att = self.obj_feat_att(obj_feats)
            obj_label_att = self.obj_label_att(obj_embed2)
            obj_att_in = F.tanh(obj_feat_att[None,:,:]+obj_label_att[:,None,:])
            #obj_feat_num = obj_feats.size(0)
            #obj_label_num  = obj_embed2.size(0)
            #obj_att_in = F.tanh(self.obj_feat_label_att(torch.cat([obj_feats[None, :, :].repeat(obj_label_num,1,1),obj_embed2[:, None, :].repeat(1,obj_feat_num,1)],-1)))
            obj_att_pre = self.att_weight(obj_att_in)[:,:,0]
            ind_label_feat = im_inds[:,None] == feat_im_inds[None,:]
            ind_label_feat = ind_label_feat.type_as(obj_att_pre)
            obj_att = obj_att_pre
            obj_att_exp = torch.exp(obj_att) * ind_label_feat
            obj_att_w = obj_att_exp / (obj_att_exp.sum(1)+1e-8)[:,None]
            #obj_att_w = F.softmax(obj_att, dim=1)
            #obj_att_w = obj_att_w * ind_label_feat
            #obj_att_w = obj_att_w/(obj_att_w.sum(1)+1e-8)[:,None]
            obj_feats = torch.sum( obj_att_w[:,:,None] * obj_feats[None,:,:],dim=1)

        if self.edge_ctx_type == 'union':
            obj_embed2_1 = obj_embed2[rel_inds[:, 1]]
            obj_embed2_2 = obj_embed2[rel_inds[:, 2]]
            obj_embed2 = torch.cat([obj_embed2_1, obj_embed2_2], -1)
            obj_feats = union_feat
            confidence = adj_dis_softmax
            box_priors = union_box

            im_inds = rel_inds[:, 0]
            obj_labels = None
            if self.training:
                obj_labels = rel_label
            inp_feats = torch.cat((obj_embed2, obj_feats), 1)
            # Sort by the confidence of the maximum detection.

            perm, inv_perm, ls_transposed = self.sort_rois(im_inds, confidence, box_priors)
        else:
            inp_feats = torch.cat((obj_embed2, obj_feats), 1)
            # Sort by the confidence of the maximum detection.
        ...
        rel_dists = None
        rel_preds = None
        rel_decoder_rep = None
        if self.edge_ctx_type == 'union':
            rel_inds_r = rel_inds[perm]
            s_obj_feats_1 = s_obj_feats[rel_inds_r[:, 1]]
            s_obj_feats_2 = s_obj_feats[rel_inds_r[:, 2]]
            edge_reps = torch.cat([s_obj_feats_1, s_obj_feats_2, edge_reps], -1)


            decoder_inp = PackedSequence(
                edge_reps,
                ls_transposed)
            rel_dists, rel_preds, rel_decoder_rep = self.decoder_edge_rnn(
                decoder_inp,  # obj_dists[perm],
                labels=obj_labels[perm] if obj_labels is not None else None,
                boxes_for_nms=box_priors[perm] if box_priors is not None else None,
                rel_inds=rel_inds_r,
                type=self.edge_ctx_type,
                obj_preds=obj_preds,

            )
            rel_preds = rel_preds[inv_perm]
            rel_decoder_rep = rel_decoder_rep[inv_perm]
            '''
            rel_dists = self.decoder_edge_linear(edge_reps)
            '''
            rel_dists = rel_dists[inv_perm]

        # now we're good! unperm
        edge_ctx = edge_reps[inv_perm]

        return edge_ctx, obj_feats, obj_att_w, rel_dists, rel_preds, rel_decoder_rep
    ...
    def obj_ctx(self, obj_feats, obj_dists, im_inds, obj_labels=None, box_priors=None, boxes_per_cls=None
        ...
        if self.with_gcn and self.where_gcn == 'stack':
            ...
            adj_dist_owbias_obj = self.from_pre_to_mat(obj_pre=adj_pre, sub_pre=adj_pre, rel_inds_graph=rel_inds,
                                                   ...
                                                   obj_preds=obj_preds, num=1, use_bias=not(self.type_gcn == 'gat'))
        ...
        obj_dists, obj_preds, decoder_rep = self.decoder_rnn(
            decoder_inp, #obj_dists[perm],
            labels=obj_labels[perm] if obj_labels is not None else None,
        ...
        return obj_dists, obj_preds, encoder_rep, decoder_rep, adj_dist_owbias_obj
    ...
    def obj_gcn(self, obj_feats, im_inds, rel_inds, adj_mat, num_layer=0,
        ...
        x = F.relu(self.obj_gc1(obj_feats, spare_adj_mat))
    ...
    def from_pre_to_mat(self, obj_pre, sub_pre, rel_inds_graph, vr_graph, rel_inds_offset_graph, obj_preds, num=0,
                        use_bias=None ):
        if num == 0:
        ...
        if self.use_vision:
            ...
            if self.limit_vision:
                # exact value TBD
                subj_rep_graph_rel = torch.cat((subj_rep_graph_rel[:,:2048] * vr_obj_graph[:,:2048], subj_rep_graph_rel[:,2048:]), 1)
                obj_rep_graph_rel = torch.cat((obj_rep_graph_rel[:,:2048] * vr_obj_graph[:,:2048], obj_rep_graph_rel[:,2048:]), 1)
            else:
                subj_rep_graph_rel = subj_rep_graph_rel * vr_obj_graph[:,:2048]
                obj_rep_graph_rel = obj_rep_graph_rel * vr_obj_graph[:, :2048]
    ...
    def gcn_ctx(self, ctx_feat, adj_mat, im_inds, rel_inds, num_layer=0, obj_preds=None,
        ...
        x = F.relu(self.gc[num_layer](ctx_feat, spare_adj_mat))
        x = F.dropout(x, self.dropout_rate, training=self.training)
    ...
    def forward(self, obj_fmaps, obj_logits, im_inds, obj_labels=None,
                ...
                gt_adj_mat=None, rel_label=None):
        """
        ...
        f_mean_map = f_map.view(f_map.size(0),f_map.size(1),-1).mean(-1)
        f_mean_map = f_mean_map[im_inds]
        adj_dist_owbias = None
        adj_dist_owbias_obj = None
        mul_dist = None
        mul_rel_dist = None
        obj_ctx_gcn = None
        if self.nl_mul > 0:
            '''
            mul_dist_per, mul_obj_preds, mul_obj_ctx, mul_decoder_rep = self.mul_ctx(
                obj_pre_rep,
                obj_logits,
                im_inds,
                obj_labels,
            ...
            mul_dist = self.max_pooling_boj(mul_dist_per, num_box)
            obj_pre_rep = torch.cat((obj_pre_rep, mul_dist_per), 1)
            '''
            mul_dist_all, mul_state_all = self.mul_ctx_T(obj_fmaps, im_inds, num_obj_per)
            mul_dist = mul_dist_all.max(1)[0]
            mul_state = mul_state_all.max(1)[0]
            # mul_dist = self.max_pooling_boj_T(mul_dist_all, num_box)
            obj_pre_rep = torch.cat((obj_pre_rep, mul_dist), 1)
        ...
        if self.with_gcn and self.where_gcn == 'parall':
            if (not self.with_gt_adj_mat) or self.type_gcn == 'gat':
                ...
                adj_dist_owbias_obj = self.from_pre_to_mat(obj_pre=adj_pre, sub_pre=adj_pre, rel_inds_graph=rel_inds,
                                                           vr_graph=union_feat,
                                                           rel_inds_offset_graph=rel_inds_offset,
                                                           obj_preds=obj_preds, num=1,
                                                           use_bias=not (self.type_gcn == 'gat'))
        ...
        if self.nl_obj > 0 and self.mode != 'predcls_nongtbox':
            ...
            obj_dists2, obj_preds, obj_ctx, decoder_rep, adj_dist_owbias_obj_t = self.obj_ctx(
                obj_pre_rep,
                obj_logits,
                im_inds,
                obj_labels,
        ...
        if self.mode == 'detclass':
            return obj_dists2, obj_preds, None, None, None, \
        ...
        if self.with_adj_mat and self.where_gcn == 'parall':
            ...
            if not self.with_gt_adj_mat:
                adj_dist_owbias = self.from_pre_to_mat(obj_pre=adj_pre, sub_pre=adj_pre, rel_inds_graph=rel_inds,
                                                       vr_graph=union_feat,
                                                       rel_inds_offset_graph=rel_inds_offset,
                                                       obj_preds=obj_preds, num=0)
            if self.with_gt_adj_mat:
                adj_dis_softmax = gt_adj_mat
            else:
                adj_dis_softmax = F.softmax(adj_dist_owbias, -1)[:, 1]
        edge_ctx = None
        obj_feat_att = None
        obj_feat_att_w = None
        union_boxs = self.get_union_box(box_priors, rel_inds)
        union_feat_nms = union_feat
        union_boxs_nms = union_boxs
        adj_dis_softmax_nms = adj_dis_softmax
        rel_inds_nms = rel_inds
        rel_label_nms = rel_label
        keep_union = None
        if self.edge_ctx_type == 'union' and self.nms_union:

            keep_union = apply_nms(adj_dis_softmax, union_boxs,
                                     pre_nms_topn=adj_dis_softmax.size(0), post_nms_topn=adj_dis_softmax.size(0),
                                     nms_thresh=0.3)

            union_boxs_nms = union_boxs[keep_union]
            union_feat_nms = union_feat[keep_union]
            adj_dis_softmax_nms = adj_dis_softmax[keep_union]
            rel_inds_nms = rel_inds[keep_union]
            if self.training:
                rel_label_nms = rel_label[keep_union]
        rel_dists = None
        if self.nl_edge > 0:
            edge_ctx, obj_feat_att, obj_feat_att_w, \
            rel_dists, rel_preds, rel_decoder_rep = self.edge_ctx(
                ...
                feat_im_inds=obj_feat_im_inds,
                union_feat=union_feat_nms,
                union_box=union_boxs_nms,
                rel_inds=rel_inds_nms,
                adj_dis_softmax=adj_dis_softmax_nms,
                rel_label=rel_label_nms,
            )
        elif self.nl_edge == 0:
            edge_ctx = obj_ctx
        ...
        if self.with_gcn:
            ...
            if self.where_gcn == 'stack':
                if self.with_adj_mat :
                    ...
                    adj_pre = edge_ctx
                    if not self.with_gt_adj_mat:
                        adj_dist_owbias = self.from_pre_to_mat(obj_pre=adj_pre, sub_pre=adj_pre,
            ...
            # print('adj_dis_softmax',adj_dis_softmax.size())
            edge_sub_ctx = edge_ctx
            edge_obj_ctx = edge_ctx

            ''' '''
            if self.pass_in_obj_feats_to_gcn:
                pred_embed3 = self.obj_embed3(obj_preds)
            ...
            '''
            adj_dist_owbias_1 = self.from_pre_to_mat(obj_pre=obj_gcn_ctx, sub_pre=sub_gcn_ctx,
                                                   rel_inds_graph=rel_inds,
                                                   vr_graph=union_feat,
                                                   rel_inds_offset_graph=rel_inds_offset,
                                                   obj_preds=obj_preds,
            ...
            adj_dis_softmax_1 = F.softmax(adj_dist_owbias_1, -1)

            #gtc_input = torch.cat([edge_ctx,f_mean_map],-1)
            gtc_input = edge_ctx
            if self.pass_in_obj_feats_to_gcn:

                gtc_input = torch.cat((obj_pre_rep3, edge_ctx), -1)
            if self.pass_embed_togcn:

                gtc_input = torch.cat((pred_embed3, edge_ctx), -1)

            edge_sub_ctx, edge_obj_ctx = self.gcn_ctx(
                gtc_input,
                adj_mat=adj_dis_softmax[:, 1]+adj_dis_softmax_1[:, 1]+adj_dis_softmax_obj[:, 1],
                im_inds=im_inds,
                rel_inds=rel_inds,
                rel_inds_offset=rel_inds_offset,
                num_layer=1,
            ...
            edge_ctx = sub_gcn_ctx + obj_gcn_ctx + edge_ctx
            edge_sub_ctx = edge_sub_ctx + edge_ctx
            edge_obj_ctx = edge_obj_ctx + edge_ctx
            '''
            adj_dist_owbias_1 = adj_dist_owbias_obj


        return obj_dists2, obj_preds, edge_ctx, edge_sub_ctx, edge_obj_ctx, \
               obj_feat_att, obj_feat_att_w, adj_dist_owbias, adj_dist_owbias_1, keep_union, rel_inds_nms, \
               rel_dists
...
class RelModel(nn.Module):
    ...
    def __init__(self, classes, rel_classes, mode='sgdet', num_gpus=1, use_vision=True, require_overlap_det=True,
        ...
        self.union_boxes = UnionBoxesAndFeats(pooling_size=self.pooling_size, stride=16,
                                              dim=1024 if use_resnet else 512, use_resnet=self.use_resnet, compress_union=self.compress_union)
    ...
    def obj_feature_map(self, features, rois):
        ...
        if not self.use_resnet:
            ...
            return torch.cat([self.roi_fmap_obj(feature_pool).mean(3).mean(2), feature_pool.mean(3).mean(2)], -1)
    ...
    def forward(self, x, im_sizes, image_offset,
        ...
        if self.training and result.rel_labels_graph is None and self.mode == 'sgdet':
        ...
        if result.rel_labels_graph is None:
            rel_label = None
        else:
        ...
        if self.mode != 'sgdet':
            ...
            gt_adj_mat = None
        result.rm_obj_dists, result.obj_preds, edge_ctx, edge_sub_ctx, edge_obj_ctx, obj_feat_att, obj_feat_att_w, \
        adj_mat, adj_mat_1, keep_union, rel_inds_nms, rel_dists = self.context(
            result.obj_fmap,
        ...
        result.pre_adj_mat = adj_mat
        result.pre_adj_mat_1 = adj_mat_1
        ...
        if self.use_bias and self.edge_ctx_type != 'union':
            ...
            result.rel_dists = result.rel_dists + self.freq_bias.index_with_labels(torch.stack((
                result.obj_preds[obj_rel_ind[:, 1]],
                result.obj_preds[obj_rel_ind[:, 2]],
            ), 1))
            '''
            result.rel_dists = result.rel_dists + F.dropout(self.freq_bias.index_with_labels(torch.stack((
                result.obj_preds[obj_rel_ind[:, 1]],
                result.obj_preds[obj_rel_ind[:, 2]],
            ), 1)),self.dropout_rate, training=self.training)
            '''
        '''
        if self.with_adj_mat and self.nl_adj == 0:

            result.pre_adj_mat, pre_adj_bist = self.context.from_pre_to_mat(
                adj_pre=edge_ctx, rel_inds_graph=rel_inds_graph, vr_graph=vr_graph,
                rel_inds_offset_graph=rel_inds_offset_graph, obj_preds=result.obj_preds
            )
        '''
        if self.training:
            return result
        ...
        result.pre_adj_mat = F.softmax(result.pre_adj_mat, dim=-1)
        result.pre_adj_mat_1 = F.softmax(result.pre_adj_mat_1, dim=-1)
        ...
        gt_adj_mat = result.gt_adj_mat_graph[rel_inds_graph[:, 1], \
                                                 rel_inds_offset_graph[:, 2]].type(torch.cuda.LongTensor)
        return filter_dets(im_inds, bboxes, result.obj_scores,
                           result.obj_preds, result.pre_adj_mat[:,1], result.pre_adj_mat_1[:,1], rel_inds_rel[:, 1:],
                           rel_rep,
