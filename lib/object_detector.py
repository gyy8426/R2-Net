...
from lib.fpn.psroi_pooling.functions.psroi_pooling import PSRoIPoolingFunction
from lib.pytorch_misc import enumerate_by_image, gather_nd, diagonal_inds, Flattener
from torchvision.models.vgg import vgg16
from torchvision.models.resnet import resnet101
# from Detectron_pytorch.lib.modeling.FPN import fpn_ResNet101_conv5_body
# from Detectron_pytorch.lib.modeling import rpn_heads
# from Detectron_pytorch.lib.modeling import fast_rcnn_heads
...
class Result(object):
    ...
    def __init__(self, od_obj_dists=None, rm_obj_dists=None,
                 ...
                 rpn_scores=None, rpn_box_deltas=None, rel_labels_graph=None,
                 im_inds=None, fmap=None, rel_dists=None, rel_inds=None,
                 rel_rep=None, rel_labels_offset_graph=None, rel_labels_rel=None, rel_labels_offset_rel=None,
                 id_true_rel=None, id_neg_rel=None, id_tp_rel=None):
...
class ObjectDetector(nn.Module):
    ...
    def __init__(self, classes, rel_classes=51, mode='rpntrain', num_gpus=1, nms_filter_duplicates=True,
                 max_per_img=64, backbone_type='vgg', thresh=0.05,
                 bg_num_graph=-1, bg_num_rel=-1, neg_time=0,
                 with_gt_adj_mat=None, roipooling='align'):
        """
        ...
        elif self.backbone_type.find('resnet') != -1 and self.backbone_type.find('fpn') == -1:  # Deprecated
            self.features, self.roi_fmap = load_resnet()
        ...
        if self.backbone_type.find('fpn') != -1:
            self.FPN_ON = True
            self.Conv_Body = fpn_ResNet101_conv5_body()
            ...
            # BBOX Branch
            self.Box_Head = fast_rcnn_heads.roi_2mlp_head(
    ...
    def obj_feature_map(self, features, rois):
        ...
        if not self.use_resnet:
            ...
            return self.roi_fmap(feature_pool).mean(3).mean(2)
    ...
    def rpn_boxes(self, fmap, im_sizes, image_offset, gt_boxes=None, gt_classes=None, gt_rels=None,
        ...
        """
        if self.FPN_ON:
            print('Wait for the implementation!')
            rpn_ret = self.RPN(fmap, im_sizes, roidb)
            fg_inds = np.where(rpn_ret['labels_int32'] > 0)[0]
            rois = rpn_ret['rois'][fg_inds]
            roi_probs = rpn_ret['roi_probs'][fg_inds]
            rois = filter_roi_proposals(rois, roi_probs,
        ...
        if self.training:
            ...
            '''
            rpn_scores : [num_rois, 2] array of yes/no
            rpn_box_deltas : [num_rois, 4] array of predicted boxes
            '''
    ...
    def gt_boxes(self, fmap, im_sizes, image_offset, gt_boxes=None, gt_classes=None, gt_rels=None,
        ...
        if gt_rels is not None and self.training:
            '''
            rois, labels, rel_labels_graph, rel_labels_offset_graph = proposal_assignments_gtbox(
                rois.data, gt_boxes.data, gt_classes.data, gt_rels.data, image_offset,
                fg_thresh=0.5, time_bg=self.bg_num_graph)
            if self.bg_num_graph == self.bg_num_rel:
                rois_rel, labels_rel, rel_labels_rel, rel_labels_offset_rel = \
                rois.clone(), labels.clone(), rel_labels_graph.clone(), rel_labels_offset_graph.clone()
            else:
                rois_rel, labels_rel, rel_labels_rel, rel_labels_offset_rel = proposal_assignments_gtbox(
                    rois.data, gt_boxes.data, gt_classes.data, gt_rels.data, image_offset,
                    fg_thresh=0.5, time_bg=self.bg_num_rel)
            '''
            max_num_bg = int(max(self.bg_num_graph, self.bg_num_rel))
            if self.bg_num_graph == -1 or self.bg_num_rel == -1:
                max_num_bg = -1
            rois, labels, rel_labels_fg, rel_labels_offset_fg = proposal_assignments_gtbox(
                rois.data, gt_boxes.data, gt_classes.data, gt_rels.data, image_offset,
                fg_thresh=0.5, time_bg=0, time_fg=1)
            rois, labels, rel_labels_bg, rel_labels_offset_bg = proposal_assignments_gtbox(
                rois.data, gt_boxes.data, gt_classes.data, gt_rels.data, image_offset,
                fg_thresh=0.5, time_bg=max_num_bg, time_fg=0)
            if self.neg_time > 0:
                rois, labels, rel_labels_neg, rel_labels_offset_neg = proposal_assignments_gtbox(
                    rois.data, gt_boxes.data, gt_classes.data, gt_rels.data, image_offset,
                    fg_thresh=0.5, time_bg=0, time_fg=0, time_neg=self.neg_time)
            max_num_bg = int(rel_labels_bg.shape[0] / (1.0 * rel_labels_fg.shape[0]))
            ...
            #num_bg_graph = int((self.bg_num_graph / (1.0 * max_num_bg)) * max_num_bg)
            #num_bg_rel = int((self.bg_num_rel / (1.0 * max_num_bg)) * max_num_bg)
            #print('num_bg_rel: ',num_bg_rel, 'num_bg_graph: ',num_bg_graph)
            ...
            if self.neg_time > 0:
                rel_labels_rel = torch.cat([rel_labels_rel, rel_labels_neg],0)
                rel_labels_offset_rel = torch.cat([rel_labels_offset_rel, rel_labels_offset_neg],0)
                rel_labels_graph = torch.cat([rel_labels_graph, rel_labels_neg], 0)
                rel_labels_offset_graph = torch.cat([rel_labels_offset_graph, rel_labels_offset_neg], 0)

            _, perm_rel = torch.sort(rel_labels_rel[:, 0] * (gt_boxes.size(0) ** 2) +
                                 rel_labels_rel[:, 1] * gt_boxes.size(0) + rel_labels_rel[:, 2])
            rel_labels_rel = rel_labels_rel[perm_rel]
            rel_labels_offset_rel = rel_labels_offset_rel[perm_rel]


            _, perm_graph = torch.sort(rel_labels_graph[:, 0] * (gt_boxes.size(0) ** 2) +
                                       rel_labels_graph[:, 1] * gt_boxes.size(0) + rel_labels_graph[:, 2])
            rel_labels_graph = rel_labels_graph[perm_graph]
            rel_labels_offset_graph = rel_labels_offset_graph[perm_graph]

            num_true_rel = rel_labels_fg.shape[0]
            _, num_true_rel_ind = torch.sort(perm_rel)
            num_true_rel_ind = num_true_rel_ind[:num_true_rel]
            if self.neg_time > 0:
                num_neg_rel = rel_labels_neg.shape[0]
                _, num_ind_1 = torch.sort(perm_rel)
                num_neg_rel_ind = num_ind_1[num_neg_rel:]
                num_tp_rel_ind = num_ind_1[:num_neg_rel]
            else:
                num_neg_rel_ind = None
                num_tp_rel_ind = None
        else:
            labels = gt_classes[:, 1]
            rel_labels_graph = None
            rel_labels_offset_graph = None
            rel_labels_rel = None
            rel_labels_offset_rel = None
            num_true_rel_ind = None
            num_neg_rel_ind = None
            num_tp_rel_ind = None

        return rois, labels, None, None, None, \
               rel_labels_graph, rel_labels_offset_graph, rel_labels_rel, rel_labels_offset_rel,\
               num_true_rel_ind, num_neg_rel_ind, num_tp_rel_ind
    ...
    def forward(self, x, im_sizes, image_offset,
        ...
        """
        if self.FPN_ON:
            fmap = self.Conv_Body(x)
        else:
            fmap = self.feature_map(x)

        # Get boxes from RPN
        rois, obj_labels, bbox_targets, rpn_scores, rpn_box_deltas, \
        rel_labels_graph, rel_labels_offset_graph, rel_labels_rel, rel_labels_offset_rel, \
        id_true_rel, id_neg_rel, id_tp_rel = \
            self.get_boxes(fmap, im_sizes, image_offset, gt_boxes,
                           gt_classes, gt_rels, train_anchor_inds, proposals=proposals)
        '''
        rois: [num_rois, 5]
        obj_labels: [num_rois] array of labels
        bbox_targets: [num_rois, 4] array of targets for the labels.
        rpn_scores : [num_rois, 2] array of yes/no
        rpn_box_deltas : [num_rois, 4] array of predicted boxes
        '''
        ...
        if (not self.training and not self.mode == 'gtbox') or self.mode in ('proposals', 'refinerels'):
            ...
            nms_boxes  [#nms, #classes+1, 4]. classid=0 is the box prior.
            inds [#nms] im_id
            '''
            #nms_imgs:  img_id after applying nms
        ...
        return Result(
            ...
            rel_labels_graph=rel_labels_graph,
            rel_labels_offset_graph=rel_labels_offset_graph,
            rel_labels_rel=rel_labels_rel,
            rel_labels_offset_rel=rel_labels_offset_rel,
            im_inds=im_inds,
            fmap=fmap if return_fmap else None,
            id_true_rel=id_true_rel,
            id_neg_rel=id_neg_rel,
            id_tp_rel=id_tp_rel,
        )
...
def load_resnet(use_dropout=True, use_relu=True, use_linear=True, pretrained=True):
