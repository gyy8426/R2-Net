    ...
    if conf.cache is None or not os.path.exists(conf.cache):
        ...
        for i, pred_entry in enumerate(tqdm(all_pred_entries)):
            ...
            det_res = (pred_entry['pred_boxes'],pred_entry['pred_classes'],
                       pred_entry['obj_scores'],pred_entry['pred_rels'],
                       pred_entry['rel_scores'],pred_entry['pred_adj_mat_rel'],
                       pred_entry['pred_boxes'],pred_entry['pred_adj_mat_obj'])
...
def val_batch(batch_num, b, evaluator, thrs=(20, 50, 100), det_res=None):
    ...
    for fiveple in matching_pred5ples:
        ...
        edges_adj[(head_name, tail_name)] = pred_adj_mat_obj_i[fiveple[0],fiveple[1]]
    ...
    for gt in gt_5ples[np.setdiff1d(np.arange(gt_5ples.shape[0]), has_match)]:
        ...
        #missededges_adj[tuple(namez)] = pred_adj_mat_obj_i[gt[0],gt[1]]
    ...
    for fiveple in pred_5ples[np.setdiff1d(np.arange(pred_5ples.shape[0]), matching_pred5ples)]:
        ...
        if fiveple[0] in pred_ind2name:
            if fiveple[1] in pred_ind2name:
                ...
                badedges_adj[(pred_ind2name[fiveple[0]], pred_ind2name[fiveple[1]])] = pred_adj_mat_obj_i[fiveple[0],fiveple[1]]
    ...
    pathname = os.path.join(conf.DATA_PATH,'qualitative', id)
