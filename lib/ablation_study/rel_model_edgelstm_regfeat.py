...
class LinearizedContext(nn.Module):
    ...
    def __init__(self, classes, rel_classes, mode='sgdet',
        ...
        if self.with_gcn:
            ...
            for i in range(self.num_gcn_layer):
                ...
                #self.gc2_list.append(GraphConvolution(self.hidden_dim, self.hidden_dim))
                '''
    ...
    def obj_ctx(self, obj_feats, obj_dists, im_inds, obj_labels=None, box_priors=None, boxes_per_cls=None,
        ...
        if self.with_gcn and self.where_gcn == 'stack':
            ...
            if not self.with_gt_adj_mat:
                ...
                if ae_adj_mat_obj is not None:
                    adj_dis_softmax_obj_1 = torch.log(ae_adj_mat_obj)
                    adj_dis_softmax_obj_0 = torch.log(1.0 - ae_adj_mat_obj)
                    adj_dist_owbias_obj = torch.cat([adj_dis_softmax_obj_0[:, None], adj_dis_softmax_obj_1[:, None]], -1)
    ...
    def obj_gcn(self, obj_feats, im_inds, rel_inds, adj_mat, num_layer=0,
        ...
        x1 = F.elu(self.obj_gc1(obj_feats, spare_adj_mat))
        x1 = F.dropout(x1, self.dropout_rate, training=self.training)
        #x1 = obj_feats + x1
        #x2 = F.elu(self.obj_gc2(x1, spare_adj_mat))
        #x2 = F.dropout(x2, self.dropout_rate, training=self.training)
        #x2 = x2 + x1
        '''
    ...
    def gcn_ctx(self, ctx_feat, adj_mat, im_inds, rel_inds, num_layer=0, obj_preds=None,
        ...
        x1 = F.elu(self.gc[0](ctx_feat, spare_adj_mat))
        x1 = F.dropout(x1, self.dropout_rate, training=self.training)
        #x1 = ctx_feat + x1
        #x2 = F.elu(self.gc2[0](x1, spare_adj_mat))
        #x2 = F.dropout(x2, self.dropout_rate, training=self.training)
        #x2 = x2 + x1
    ...
    def forward(self, obj_fmaps, obj_logits, im_inds, obj_labels=None,
                ...
                ae_adj_mat_rel=None, ae_adj_mat_obj=None,
                ):
        """
            ...
            if self.where_gcn == 'stack':
                if self.with_adj_mat :
                    ...
                    if not self.with_gt_adj_mat:
                        ...
                        if ae_adj_mat_rel is not None:
                            adj_dist_owbias_rel_1 = torch.log(ae_adj_mat_rel)
                            adj_dist_owbias_rel_0 = torch.log(1.0 - ae_adj_mat_rel)
                            adj_dist_owbias_rel = torch.cat([adj_dist_owbias_rel_0[:,None], adj_dist_owbias_rel_1[:,None]],-1)
...
class RelModel(nn.Module):
    ...
    def forward(self, ids, x, im_sizes, image_offset,
        ...
        if ae_adj_mat_rel is not None:
            ae_pred_rel_inds_sum = ae_pred_rel_inds[:,0] * (ae_pred_rel_inds[:,1].max()+1) + ae_pred_rel_inds[:,1]
            re1, sort_ind1 = torch.sort(ae_pred_rel_inds_sum)
            ae_pred_rel_inds_s1 = ae_pred_rel_inds[sort_ind1]
            ae_adj_mat_rel_r = ae_adj_mat_rel[sort_ind1]
            ae_adj_mat_obj_r = ae_adj_mat_obj[sort_ind1]
        else:
            ae_adj_mat_rel_r = None
            ae_adj_mat_obj_r = None
        ...
        adj_mat_rel, adj_mat_obj, keep_union, rel_inds_nms, rel_dists = self.context(
            ...
            ae_adj_mat_rel=ae_adj_mat_rel_r, ae_adj_mat_obj=ae_adj_mat_obj_r,
        )
