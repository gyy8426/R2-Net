...
class LinearizedContext(nn.Module):
    ...
    def __init__(self, classes, rel_classes, mode='sgdet',
                 ...
                 nl_obj=2, nl_obj_gcn=2, nl_edge=2, dropout_rate=0.2, order='confidence',
                 obj_feats_to_decoder=True,
                 obj_feats_to_edge=True,
                 obj_feats_to_gcn=False,
                 with_adj_mat=False, use_bias=True, use_tanh=True,
                 limit_vision=True, use_vision=True,
                 bg_num_graph=-1,
                 with_gcn=False, fb_thr=0.5, with_biliner_score=False,
                 gcn_adj_type='hard', num_gcn_layer=1,
                 where_gcn='stack', with_gt_adj_mat=False, type_gcn='normal',
                 ):
        ...
        self.obj_feats_to_decoder = obj_feats_to_decoder
        self.obj_feats_to_edge = obj_feats_to_edge
        self.obj_feats_to_gcn = obj_feats_to_gcn
        ...
        if self.nl_obj > 0 and self.mode != 'predcls_nongtbox':
            ...
            if self.with_gcn and self.where_gcn == 'front':
                obj_ctx_rnn_indim = self.hidden_dim
        ...
        if self.nl_obj == 0 or self.with_gcn:
            if self.where_gcn == 'front' or self.where_gcn == 'parall':
                obj_gcn_input_dim = self.obj_dim + self.embed_dim + 128
            if self.where_gcn == 'back':
            ...
            for i in range(self.num_gcn_layer):
                self.gc_list.append(GraphConvolution(obj_gcn_input_dim, self.hidden_dim))
                '''
                self.gc_re_list.append(gcn_layer(gcn_input_dim, self.hidden_dim))
                '''
            self.obj_gcn = nn.Sequential(*self.gc_list)
            '''
            self.obj_gcn_re = nn.Sequential(*self.gc_re_list)
            '''
            if self.nl_obj == 0:
        ...
        if (self.with_adj_mat and not self.with_gt_adj_mat) or self.type_gcn == 'gat':
            ...
            if self.with_gcn and (self.where_gcn == 'parall' or self.where_gcn == 'front'):
                post_lstm_in = post_lstm_in + self.hidden_dim
            elif self.with_gcn and self.where_gcn == 'back':
                post_lstm_in = self.hidden_dim
            self.obj_post_lstm_graph_obj = nn.Linear(post_lstm_in, self.pooling_dim)
            self.obj_post_lstm_graph_sub = nn.Linear(post_lstm_in, self.pooling_dim)
            '''
            ...
            '''
            self.obj_post_lstm_graph_obj.weight = torch.nn.init.xavier_normal(self.obj_post_lstm_graph_obj.weight,
                                                                              gain=1.0)
            self.obj_post_lstm_graph_sub.weight = torch.nn.init.xavier_normal(self.obj_post_lstm_graph_sub.weight,
                                                                              gain=1.0)
            ...
            self.edge_post_lstm_graph_obj = nn.Linear(post_lstm_dim, self.pooling_dim)
            self.edge_post_lstm_graph_sub = nn.Linear(post_lstm_dim, self.pooling_dim)
            '''
            ...
            '''
            self.edge_post_lstm_graph_obj.weight = torch.nn.init.xavier_normal(self.edge_post_lstm_graph_obj.weight,
                                                                               gain=1.0)
            self.edge_post_lstm_graph_sub.weight = torch.nn.init.xavier_normal(self.edge_post_lstm_graph_sub.weight,
                                                                               gain=1.0)
            if self.with_biliner_score:
                self.obj_graph_bilinear = nn.Linear(self.pooling_dim, 2, bias=False)
                self.obj_graph_bilinear.weight = torch.nn.init.xavier_normal(self.obj_graph_bilinear.weight, gain=1.0)
                self.edge_graph_bilinear = nn.Linear(self.pooling_dim, 2, bias=False)
                self.edge_graph_bilinear.weight = torch.nn.init.xavier_normal(self.edge_graph_bilinear.weight, gain=1.0)
            else:
        ...
        if self.nl_edge > 0:
            ...
            if self.obj_feats_to_edge:
                input_dim += self.obj_dim
            if self.with_gcn:
                input_dim += self.hidden_dim
                if self.where_gcn == 'front':
                    input_dim = self.hidden_dim
        ...
        if self.with_gcn:
            ...
            if self.where_gcn == 'front' or self.where_gcn == 'parall':
                edge_gcn_input_dim = self.embed_dim + self.hidden_dim + self.hidden_dim
            if self.where_gcn == 'back':
                edge_gcn_input_dim = self.hidden_dim

            if self.obj_feats_to_gcn:
                edge_gcn_input_dim += self.obj_dim + self.embed_dim + 128
            ...
            '''
            for i in range(self.num_gcn_layer):
                self.gc_list.append(GraphConvolution(edge_gcn_input_dim, self.hidden_dim))
                '''
                self.gc_re_list.append(gcn_layer(gcn_input_dim, self.hidden_dim))
                '''
            self.edge_gcn = nn.Sequential(*self.gc_list)
            '''
            self.edge_gc_re = nn.Sequential(*self.gc_re_list)
            '''
    ...
    def obj_ctx(self, obj_feats, obj_dists, im_inds, obj_labels=None, box_priors=None, boxes_per_cls=None,
        ...
        if self.with_gcn and self.where_gcn == 'back':
            ...
            adj_dist_owbias_obj = self.from_pre_to_mat(obj_pre=adj_pre, sub_pre=adj_pre, rel_inds_graph=rel_inds,
                                                       vr_graph=union_feat,
                                                       obj_preds=adj_obj_preds, where='obj')
        ...
        decoder_inp = PackedSequence(
            torch.cat((obj_inp_rep, encoder_rep), 1) if self.obj_feats_to_decoder else encoder_rep,
            ls_transposed)
    ...
    def obj_rel_embed(self, obj_feats, im_inds, rel_inds, adj_mat, num_layer=0,
        ...
        x = F.elu(self.obj_gcn[num_layer](obj_feats, spare_adj_mat))
    ...
    def from_pre_to_mat(self, obj_pre, sub_pre, rel_inds_graph, vr_graph, \
                        obj_preds, where='obj'):

        if where == 'obj':
            subj_rep_graph = self.obj_post_lstm_graph_sub(sub_pre)
            obj_rep_graph = self.obj_post_lstm_graph_obj(obj_pre)

        if where == 'edge':
            subj_rep_graph = self.edge_post_lstm_graph_sub(sub_pre)
            obj_rep_graph = self.edge_post_lstm_graph_obj(obj_pre)
        ...
        if self.with_biliner_score:
            rel_dists_graph = self.bilinear_score_graph(subj_rep_graph_rel, obj_rep_graph_rel, where)
        else:
            prod_rep_graph = subj_rep_graph_rel * obj_rep_graph_rel
            rel_dists_graph = self.rel_compress_graph(prod_rep_graph)

        if self.use_bias:
            rel_dists_graph = rel_dists_graph + self.freq_bias_graph.rel_index_with_labels(torch.stack((
    ...
    def edge_rel_embed(self, ctx_feat, adj_mat, im_inds, rel_inds, num_layer=0, obj_preds=None,
        ...
        x = F.elu(self.edge_gcn[num_layer](ctx_feat, spare_adj_mat))
        x = F.dropout(x, self.dropout_rate, training=self.training)
    ...
    def bilinear_score_graph(self, sub, obj, where='obj'):
        '''
        ...
        if where == 'edge':
            rel_dists_graph = self.edge_graph_bilinear(prod_rep_graph)
            # rel_dists_graph = F.dropout(rel_dists_graph, self.dropout_rate, training=self.training)
        return rel_dists_graph
    ...
    def forward(self, obj_fmaps, obj_logits, im_inds, obj_labels=None,
                box_priors=None, boxes_per_cls=None, union_feat=None,
                rel_inds=None, rel_inds_offset=None,
                gt_adj_mat=None, label_prior=None):
        """
        ...
        if self.with_gcn and (self.where_gcn == 'parall' or self.where_gcn == 'front'):
            ...
            obj_ctx_gcn = self.obj_rel_embed(obj_pre_rep,
                                             im_inds=im_inds,
                                             rel_inds=rel_inds,
                                             adj_mat=adj_dis_softmax_obj
                                             )
            if self.where_gcn == 'front':
                obj_ctx_input = obj_ctx_gcn
        ...
        if self.with_gcn and (self.where_gcn == 'parall' or self.where_gcn == 'front'):
            ...
            if (not self.with_gt_adj_mat) or self.type_gcn == 'gat':
                if self.mode == 'sgdet':
                    adj_edge_preds = obj_dists2.detach().max(1)[1]
                else:
                    if self.training or self.mode == 'predcls':
                        adj_edge_preds = obj_labels
                    else:
                        adj_edge_preds = obj_dists2[:, 1:].max(1)[1] + 1
                adj_dist_owbias_rel = self.from_pre_to_mat(obj_pre=adj_pre, sub_pre=adj_pre, rel_inds_graph=rel_inds,
                                                           vr_graph=union_feat,
                                                           obj_preds=adj_edge_preds, where='where',
                                                           )
            if self.with_gt_adj_mat:
                adj_dis_softmax_rel = gt_adj_mat
                if self.type_gcn == 'gat':
                    adj_dis_softmax_rel = gt_adj_mat * adj_dis_softmax_obj
            else:
                adj_dis_softmax_rel = F.softmax(adj_dist_owbias_rel, -1)[:, 1]
            gtc_input = obj_ctx
            obj_embed2 = self.obj_embed2(edge_obj_pred)
            gtc_input = torch.cat((obj_embed2, gtc_input), 1)
            sub_gcn_ctx, obj_gcn_ctx = self.edge_rel_embed(
                gtc_input,
        ...
        if self.nl_edge > 0:
            edge_ctx = self.edge_ctx(
                torch.cat((obj_fmaps, obj_ctx), 1) if self.obj_feats_to_edge else edge_ctx_input,
            ...
            if self.with_adj_mat:
                ...
                if not self.with_gt_adj_mat:
                    ...
                    adj_dist_owbias_rel = self.from_pre_to_mat(obj_pre=adj_pre, sub_pre=adj_pre,
                                                               ...
                                                               vr_graph=union_feat,
                                                               obj_preds=adj_obj_preds, where='edge',
                                                               )
                if self.with_gt_adj_mat:
                    adj_dis_softmax_rel = gt_adj_mat
                else:
        ...
        return obj_dists2, obj_preds_nozeros, obj_preds_zeros, edge_ctx, edge_sub_ctx, edge_obj_ctx, \
               adj_dist_owbias_rel, adj_dist_owbias_obj, keep_union
...
class RelModel(nn.Module):
    ...
    def __init__(self, classes, rel_classes, mode='sgdet', num_gpus=1, use_vision=True, require_overlap_det=True,
                 ...
                 nl_obj=1, nl_edge=2, nl_adj=2, order='confidence', thresh=0.01,
                 use_proposals=False, obj_feats_to_decoder=True, obj_feats_to_gcn =False,
                 obj_feats_to_edge=True,
                 rec_dropout=0.0, use_bias=True, use_tanh=True,
                 limit_vision=True, adj_embed_dim=256, with_adj_mat=False,
                 max_obj_num=65, bg_num_graph=-1, bg_num_rel=-1,
                 with_att=False,
                 with_gcn=False, fb_thr=0.5, with_biliner_score=False,
                 gcn_adj_type='hard', where_gcn = 'parall', with_gt_adj_mat=False,type_gcn='normal',
                 edge_ctx_type='obj', cosine_dis=False, test_alpha=0.5,
                 ext_feat=False, backbone_type='vgg'
                 ):
        ...
        if self.backbone_type.find('vgg') != -1:
            self.obj_dim = 4096
        if self.backbone_type.find('resnet') != -1:
            self.obj_dim = 2048
            if self.backbone_type.find('fpn') != -1:
                self.obj_dim = 256
        self.pooling_dim = pooling_dim
        self.use_bias = use_bias
        self.use_vision = use_vision
        ...
        self.context = LinearizedContext(self.classes, self.rel_classes, mode=self.mode,
                                         ...
                                         order=order,
                                         obj_feats_to_decoder=obj_feats_to_decoder,
                                         obj_feats_to_edge=obj_feats_to_edge,
                                         obj_feats_to_gcn=obj_feats_to_gcn,
                                         with_adj_mat=with_adj_mat,
                                         use_bias = use_bias,
        ...
        if self.backbone_type.find('resnet') != -1 and self.backbone_type.find('fpn') == -1:
        ...
        self.union_boxes = UnionBoxesAndFeats(pooling_size=self.pooling_size, stride=16,
                                              dim=1024, compress_union=self.compress_union)
    ...
    def obj_feature_map(self, features, rois):
        ...
        feature_pool = RoIAlignFunction(self.pooling_size, self.pooling_size, spatial_scale=1 / 16)(
                       features, rois)
        if not self.backbone_type.find('vgg') != -1:
            return self.roi_fmap_obj(feature_pool.view(rois.size(0), -1))
        else:
    ...
    def forward(self, ids, x, im_sizes, image_offset,
        ...
        result.rm_obj_dists, result.obj_preds_nozeros, result.obj_preds_zeros, \
        edge_ctx, edge_sub_ctx, edge_obj_ctx, \
        adj_mat_rel, adj_mat_obj, keep_union = self.context(
            obj_fmaps=result.obj_fmap, obj_logits=result.rm_obj_dists.detach(),
            im_inds=im_inds, obj_labels=obj_labels,
            box_priors=boxes.data, boxes_per_cls=result.boxes_all, union_feat=vr_graph,
            rel_inds=rel_inds_graph.clone(), rel_inds_offset=rel_inds_offset_graph,
            gt_adj_mat=gt_adj_mat, label_prior=result.obj_preds,
        )
