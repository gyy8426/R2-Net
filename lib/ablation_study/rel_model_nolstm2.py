...
class LinearizedContext(nn.Module):
    ...
    def __init__(self, classes, rel_classes, mode='sgdet',
        ...
        if (self.with_adj_mat and not self.with_gt_adj_mat)  or self.type_gcn == 'gat':
            ...
            if self.use_bias:
                ...
                #self.freq_bias_graph_1 = FrequencyBias(graph=True, with_bg=bg_num_graph!=0)
        ...
        if self.with_gcn:
            ...
            self.post_lstm_graph_obj = nn.Linear(gcn_input_dim, self.pooling_dim)
            self.post_lstm_graph_sub = nn.Linear(gcn_input_dim, self.pooling_dim)
            '''
    ...
    def forward(self, obj_fmaps, obj_logits, im_inds, obj_labels=None,
        ...
        obj_embed2 = self.obj_embed2(obj_preds_nozeros)

        if self.with_gcn:

            #gtc_input = torch.cat([edge_ctx,f_mean_map],-1)
            gtc_input = torch.cat([obj_ctx, obj_embed2], -1)
            if self.where_gcn == 'stack':
                if self.with_adj_mat :
                    adj_pre = gtc_input
                    if self.mode == 'sgdet':
            ...
            edge_sub_ctx = sub_gcn_ctx
            edge_obj_ctx = obj_gcn_ctx
            edge_ctx = sub_gcn_ctx + obj_gcn_ctx
