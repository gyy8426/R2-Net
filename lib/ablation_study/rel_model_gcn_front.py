...
class LinearizedContext(nn.Module):
    ...
    def __init__(self, classes, rel_classes, mode='sgdet',
        ...
        if self.with_gcn:
            ...
            if self.where_gcn == 'parall':
                gcn_input_dim += self.hidden_dim
            if self.where_gcn == 'front':
                gcn_input_dim += self.embed_dim + self.hidden_dim
            if self.pass_embed_togcn:
    ...
    def edge_ctx(self, obj_feats, obj_dists, im_inds, obj_preds, box_priors=None, obj_embed2=None):
        """
        ...
        if self.with_gcn and (self.where_gcn == 'stack' or self.where_gcn == 'parall'):
    ...
    def forward(self, obj_fmaps, obj_logits, im_inds, obj_labels=None,
        ...
        if self.mode == 'sgdet':
            ...
            edge_obj_pred = obj_preds_nozeros.clone()
        if self.with_adj_mat and (self.where_gcn == 'parall' or self.where_gcn == 'front'):
            adj_pre = obj_ctx
            if self.pass_embed_togcn:
        ...
        if self.nl_edge > 0:
            edge_ctx = self.edge_ctx(
                torch.cat((obj_fmaps, obj_ctx), 1) if self.pass_in_obj_feats_to_edge else edge_ctx_input,
