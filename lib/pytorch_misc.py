    ...
    for name, param in state_dict.items():
        if name not in own_state:
            ...
            print("Network has {} with size {}, ckpt has {}".format(name,
                                                                    ...
                                                                    param.size()))
            mismatch = True
    #own_state['context.freq_bias_graph_1.obj_baseline.weight'].copy_(state_dict['context.freq_bias_graph.obj_baseline.weight'])
...
def load_reslayer4(detector, ckpt, num_layer):
    for i in range(num_layer):
        detector.roi_fmap[i].conv1.weight.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.conv1.weight'])
        detector.roi_fmap[i].bn1.weight.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.bn1.weight'])
        detector.roi_fmap[i].bn1.bias.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.bn1.bias'])

        detector.roi_fmap[i].conv2.weight.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.conv2.weight'])
        detector.roi_fmap[i].bn2.weight.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.bn2.weight'])
        detector.roi_fmap[i].bn2.bias.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.bn2.bias'])

        detector.roi_fmap[i].conv3.weight.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.conv3.weight'])
        detector.roi_fmap[i].bn3.weight.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.bn3.weight'])
        detector.roi_fmap[i].bn3.bias.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.bn3.bias'])

        #detector.roi_fmap_obj[i].bn1.weight.data.copy_(ckpt['state_dict'][layer_name])
        detector.roi_fmap_obj[i].conv1.weight.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.conv1.weight'])
        detector.roi_fmap_obj[i].bn1.weight.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.bn1.weight'])
        detector.roi_fmap_obj[i].bn1.bias.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.bn1.bias'])

        detector.roi_fmap_obj[i].conv2.weight.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.conv2.weight'])
        detector.roi_fmap_obj[i].bn2.weight.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.bn2.weight'])
        detector.roi_fmap_obj[i].bn2.bias.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.bn2.bias'])

        detector.roi_fmap_obj[i].conv3.weight.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.conv3.weight'])
        detector.roi_fmap_obj[i].bn3.weight.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.bn3.weight'])
        detector.roi_fmap_obj[i].bn3.bias.data.copy_(ckpt['state_dict']['roi_fmap.'+str(i)+'.bn3.bias'])


        if i == 0:
            '''
                'roi_fmap.0.downsample.0.weight'
                'roi_fmap.0.downsample.1.weight'
                'roi_fmap.0.downsample.1.bias'
                'roi_fmap.0.downsample.1.running_mean'
                'roi_fmap.0.downsample.1.running_var'
            '''
            detector.roi_fmap[i].downsample[0].weight.data.copy_(ckpt['state_dict']['roi_fmap.0.downsample.0.weight'])
            detector.roi_fmap[i].downsample[1].weight.data.copy_(ckpt['state_dict']['roi_fmap.0.downsample.1.weight'])
            detector.roi_fmap[i].downsample[1].bias.data.copy_(ckpt['state_dict']['roi_fmap.0.downsample.1.bias'])

            detector.roi_fmap_obj[i].downsample[0].weight.data.copy_(ckpt['state_dict']['roi_fmap.0.downsample.0.weight'])
            detector.roi_fmap_obj[i].downsample[1].weight.data.copy_(ckpt['state_dict']['roi_fmap.0.downsample.1.weight'])
            detector.roi_fmap_obj[i].downsample[1].bias.data.copy_(ckpt['state_dict']['roi_fmap.0.downsample.1.bias'])
...
def random_choose(tensor, num, re_id=False):
...
def reli_in_rels(rel_i, reli_not_in_rels):
    for gt_reli in reli_not_in_rels:
        if (rel_i == gt_reli).all():
            return True
    return False

def generate_negative_samples(gt_rels, time_neg=1):
    entities_id = set(gt_rels[:,1]) | set(gt_rels[:,2])
    entities_id = np.array(list(entities_id))
    neg_sample = []
    for rels_i in gt_rels:
        entities_id_t = np.setdiff1d(entities_id.copy(),rels_i[1:3])
        for neg_i in np.arange(time_neg):
            sample_neg_i = None
            while (len(entities_id_t)>0):
                s_or_o = np.random.randint(2)
                t_neg_i = np.random.choice(entities_id_t)
                entities_id_t = np.setdiff1d(entities_id_t, t_neg_i)
                if s_or_o == 1:
                    sample_neg_i = np.array([rels_i[0], t_neg_i, rels_i[2], rels_i[3]])
                if s_or_o == 0:
                    sample_neg_i = np.array([rels_i[0], rels_i[1], t_neg_i, rels_i[3]])
                if reli_in_rels(sample_neg_i, gt_rels) or sample_neg_i[1] == sample_neg_i[2]:
                    continue
                else:
                    break
